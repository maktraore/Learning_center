class OnlineSchedulePdf < Prawn::Document
    ...
    def schedules_all
      ...
      Employee.all.joins(:locations).where('locations.name = ?', "Online").where(approved: true).each {|employee| 
        emp<<  [employee.full_name , employee.timeslots.where(approved: true).where(location_id:5).map{|timeslot| [timeslot.days, timeslot.start_time, timeslot.end_time].join(" ")  }.join("\n "),  
